...
/**
 * "12–14 Mar 2026" for a run, "12 Mar 2026" for a single day.
 */
...
function WorkshopCard({ workshop, purchased, onClick }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-surface-dim">
        ...
        {/* Day-count chip — the thing that makes a workshop a workshop */}
        <span className="absolute top-3 left-3 text-[10px] sm:text-xs font-semibold text-white bg-black/55 backdrop-blur-sm px-2.5 py-1 rounded-full">
          {workshop.day_count} day{workshop.day_count === 1 ? '' : 's'}
        </span>

        {isLive && (
          <span className="absolute bottom-3 left-3 flex items-center gap-1.5 text-[10px] sm:text-xs font-semibold text-white bg-error px-2.5 py-1 rounded-full shadow-sm">
      ...
      <div className="flex flex-col flex-1 p-4 sm:p-5">
        ...
        <div className="mt-auto pt-3 border-t border-border/40 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            {workshop.faculty_names?.length > 0 && (
              <>
                {workshop.faculty_photo_urls?.[0] ? (
                  <img src={workshop.faculty_photo_urls[0]} alt="" className="w-7 h-7 rounded-full object-cover shrink-0" />
                ) : (
                  <div className="w-7 h-7 rounded-full bg-primary/8 flex items-center justify-center shrink-0">
                    <span className="text-[10px] font-semibold text-primary">
                      {workshop.faculty_names[0].charAt(0).toUpperCase()}
                    </span>
                  </div>
                )}
                <span className="text-xs font-medium text-text truncate">
                  {workshop.faculty_names[0]}
                  {workshop.faculty_names.length > 1 && ` +${workshop.faculty_names.length - 1}`}
                </span>
              </>
            )}
          </div>
          ...
          <div className="shrink-0 flex items-center gap-2">
            {purchased ? (
              <span className="text-xs font-semibold text-success bg-success/8 px-2.5 py-1 rounded-full">Enrolled</span>
            ) : workshop.is_free ? (
              <span className="text-xs font-semibold text-success bg-success/8 px-2.5 py-1 rounded-full">FREE</span>
            ) : (
              <>
                <span className="text-sm font-bold text-text">{formatPrice(workshop.price)}</span>
...
export default function WorkshopList() {
  ...
  const { isWorkshopPurchased } = usePurchase();
  ...
  // Apply ?subject= once, on mount
  useEffect(() => {
    if (urlAppliedRef.current || subjects.length === 0) return;
    const subjectSlug = searchParams.get('subject');
    if (subjectSlug) selectSubjectBySlug(subjectSlug);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await getWorkshops(subjectId);
        setWorkshops(result.workshops || result || []);
        setError('');
      } catch {
  ...
  const now = new Date();
  const active = workshops.filter((w) => !w.end_date || new Date(w.end_date) > now);

  const yours = [];
  const others = [];
  active.forEach((w) => {
    if (isWorkshopPurchased(w.workshop_id)) yours.push(w);
    else others.push(w);
  });

  const hasYours = isAuthenticated && yours.length > 0;
  const total = yours.length + others.length;

  return (
    <div className="animate-fade-in-up">
      <div className="mb-6 sm:mb-10 lg:mb-12">
        <div className="mb-5">
          <p className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">Intensive</p>
          ...
          <p className="text-text-secondary text-sm sm:text-base mt-2">
            Multi-day live programmes — one booking covers every day
          </p>
        </div>
        ...
        <div className="relative" ref={specialtyRef}>
          ...
          >
            <div className="min-w-0">
              ...
              <p className="text-base sm:text-lg font-semibold text-text truncate">
      ...
      {total === 0 ? (
        <div className="text-center py-16 sm:py-24">
          ...
          <p className="text-text-secondary text-sm sm:text-base font-medium mb-1">No workshops available</p>
          <p className="text-text-tertiary text-xs sm:text-sm">Check back later for upcoming workshops</p>
        </div>
      ) : (
          ...
          {hasYours && (
            <div>
              <div className="mb-5 sm:mb-6">
                <h2 className="text-lg sm:text-xl font-semibold text-text">Your Workshops</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-5 lg:gap-6 2xl:gap-7">
                {yours.map((w) => (
                  <WorkshopCard
                    ...
                    workshop={w}
                    purchased
                    onClick={() => navigate(`/workshops/${w.workshop_id}`)}
                  />
                ))}
              </div>
            </div>
            ...
            <div>
              {hasYours && (
                <div className="border-t border-border pt-6 sm:pt-8 mb-5 sm:mb-6">
                  <h2 className="text-lg sm:text-xl font-semibold text-text">All Workshops</h2>
                </div>
              )}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-5 lg:gap-6 2xl:gap-7">
                {others.map((w) => (
